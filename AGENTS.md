...
38. Depois que a implementação for aprovada no PR, execute o archive na mesma feature branch. O workflow de archive deve localizar a entrada da Change em `docs/roadmap.md`, marcá-la como `IN_PROGRESS`/`ARCHIVE` antes do comando de archive e incluir no mesmo commit final os artefatos arquivados, as specs consolidadas, a atualização definitiva do roadmap, o README coerente com o estado final e a remoção de `.github/screenshots` do versionamento, caso haja arquivos rastreados. Remova do README menções transitórias como "em implementação", "em verificação" ou "antes do archive" referentes à Change concluída. Aguarde o CI final e só então faça merge. Não arquive diretamente na `main`.
...
44. Antes do commit final do archive, valide em conjunto que a pasta ativa da Change deixou de existir, o diretório arquivado e as specs consolidadas existem, a linha correspondente no roadmap contém `DONE`, `ARCHIVED` e a data de conclusão, o README descreve somente comportamentos e estados efetivamente atuais e `git ls-files .github/screenshots` não retorna arquivos. Qualquer divergência bloqueia o archive e o merge.

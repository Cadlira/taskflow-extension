...
### Provedores de IA configurados pelo usuário

A assistência de IA é opcional e BYOK: o usuário informa provedor, credencial e modelo, e a extensão fala diretamente com esse endereço. Não há backend do TaskFlow, chave embutida nem dependência de nuvem. Esta capability configura e verifica a conexão; nenhum dado de tarefa é enviado e nenhuma operação de geração existe.

- **Credencial em chave própria, sem cifra.** A configuração vive em `taskflow.ai`, em `{ schemaVersion: 1, config }`, decodificada por `stored-ai-config.ts` no mesmo padrão de tarefas e lixeira: chave ausente significa "nenhum provedor configurado" e estrutura desconhecida é recusada integralmente, bloqueando salvar e testar até a remoção explícita, sem sobrescrever. `chrome.storage.sync` não é usado, para que a credencial não seja replicada para os servidores do navegador nem para outros dispositivos do perfil. Cifrar com material de chave guardado no mesmo armazenamento foi recusado de propósito: quem lê uma chave lê a outra. A proteção real é o escopo — chave separada, fora do backup, fora dos logs, mascarada na interface e removível.
- **Teto de permissões com concessão por origem.** O Manifest declara `optional_host_permissions` com `https://*/*`, `http://localhost/*` e `http://127.0.0.1/*`, apenas como teto do que pode ser pedido em tempo de execução. Declarar não concede: nenhuma permissão nova aparece na instalação ou na atualização, e `permissions` não mudou. A concessão efetiva é sempre a origem específica do provedor configurado, pedida a partir de gesto do usuário e revogada ao remover a configuração. Padrões de correspondência ignoram a porta, então `http://localhost/*` cobre `:11434` e `:1234` sem enumerá-las.
- **Três provedores, dois protocolos de fio.** `OPENAI` e `ANTHROPIC` têm base fixa no domínio e nenhum campo de endereço; `CUSTOM` exige a base informada e usa o protocolo compatível com OpenAI, que é por onde entram Ollama, LM Studio e gateways próprios. A base informada só é aceita como URL absoluta com `https`, ou `http` restrito a `localhost`, `127.0.0.1` e `[::1]`, sem usuário ou senha embutidos e sem consulta nem fragmento — uma credencial nesses trechos cairia nos logs do servidor de destino. "Compatível com OpenAI" não é outro adapter: é o adapter de OpenAI com outra base.
- **Nenhuma dependência nova; `fetch` direto.** Os SDKs oficiais foram recusados por serem desenhados para servidor, exigirem habilitar o uso em navegador e incharem o pacote sem cobrir nada que `fetch` não cubra. `dependencies` continua com `vue` e `pinia`. O teste usa a listagem de modelos do provedor, com `AbortController` e limite de tempo, cancelável e com `redirect: 'error'` — um redirecionamento desviaria a credencial para outro destino. Quando o endereço não oferece a listagem, o sistema informa isso e oferece, como segunda ação explícita, um envio mínimo com conteúdo literal fixo e um token de resposta. A Anthropic recebe `x-api-key`, `anthropic-version` e `anthropic-dangerous-direct-browser-access`, sem o qual ela recusa requisições originadas de navegador.
- **A rede vive no Side Panel.** O gesto do usuário, a concessão da permissão e a interface estão ali; emitir a requisição no mesmo contexto elimina troca de mensagens e impede que um caminho de rede de IA exista em um service worker despertado por eventos. Testes verificam que o background não alcança nenhum módulo de IA, nem por importação transitiva, e que instalação, atualização, inicialização e alarme não contatam provedor algum mesmo com configuração salva e permissão concedida.
- **Motivos enumerados; corpo do provedor descartado.** Provedores ecoam trechos da chave em mensagens de credencial inválida e um gateway de terceiros pode ecoá-la por completo, então o corpo da resposta nunca é lido, renderizado nem registrado. Toda falha vira um motivo de `AiConnectionFailure` — credencial inválida, permissão ausente, origem inalcançável, listagem indisponível, tempo esgotado, resposta inesperada —, e o log carrega no máximo motivo, origem e código de estado, nunca a URL completa, cabeçalhos ou `cause` com dados da requisição. Nenhum caminho de IA passa pelo `logFailure` genérico do background.
- **Credencial fora do estado compartilhado.** Nenhuma store Pinia conhece a credencial: o adapter a lê sob demanda do repository no momento da requisição. O campo usa `type="password"`, `autocomplete="off"` e `spellcheck="false"`, nunca é preenchido com o valor armazenado ao reabrir — uma marca indica apenas que existe credencial salva — e deixá-lo intocado preserva a que está gravada. Antes do primeiro envio a uma origem, a área informa que a credencial será enviada e exige ação explícita; trocar provedor ou base reapresenta o aviso.
- **Backup e permissões.** `encodeBackupFile` monta o arquivo por lista de permissão e `replaceAll` grava somente `taskflow.tasks`, então a configuração não entra na exportação e não é criada, alterada nem removida pela restauração. Testes de regressão exportam com credencial salva e afirmam a ausência dela, do modelo, da base e do nome da chave no conteúdo serializado. `tests/architecture/layer-boundaries.test.ts` passou a barrar `fetch(` em `domain` e `application`, e `tests/manifest/manifest-permissions.test.ts` fixa exatamente o teto de `optional_host_permissions`.
...
| `optional_host_permissions` (`https://*/*`, `http://localhost/*`, `http://127.0.0.1/*`) | Teto do que a área de provedores de IA pode pedir em tempo de execução. Declarar não concede: a concessão efetiva é sempre a origem específica do provedor configurado, pedida por gesto do usuário e revogada ao remover a configuração. |

`activeTab` e `contextMenus` não exibem aviso de permissão na instalação nem na atualização. A chave `commands` não é uma permissão e não acrescentou nenhuma. `optional_host_permissions` também não exibe aviso na instalação nem na atualização, porque permissões opcionais não são concedidas nesses momentos. Não há `tabs`, `scripting`, `favicon`, `host_permissions`, `content_scripts` nem padrões `<all_urls>`. A captura não lê o conteúdo da página, não injeta código, não lê outras abas e não envia dados para fora da extensão. Sem configuração de provedor de IA salva, nenhuma origem é acessada e nenhuma requisição de rede é feita.

## Evolução futura

Backend próprio, autenticação central e dependência obrigatória de nuvem estão fora da direção do produto. Backup automático ou agendado, mesclagem de backups, criptografia do arquivo, integrações diretas opcionais, histórico por campo, refazer, dashboards, linguagem natural, recursos de IA que gerem, resumam, reescrevam ou interpretem conteúdo de tarefa e leitura de conteúdo da página além de título, URL e texto selecionado exigirão Changes próprias. Permissões como `tabs`, `scripting`, `favicon` ou acesso a hosts só devem entrar junto ao caso de uso que as exija.
